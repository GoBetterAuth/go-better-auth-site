...
# Gives admin access to update and manage the GoBetterAuth server. Only applicable if Config Manager plugin is enabled - by default it is.
...
```bash
$ go get github.com/GoBetterAuth/go-better-auth@latest
```
...
GoBetterAuth runs all migrations (core & plugins) automatically when you create a new instance so all the relevant database tables are created for you without any additional steps. This is done via Bun ORM's migration system and migration files are embedded in the binary. But GoBetterAuth also provides migration files which you can run directly against your database of choice.
